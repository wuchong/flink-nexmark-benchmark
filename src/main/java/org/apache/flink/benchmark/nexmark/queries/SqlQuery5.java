    ...
    private static final String TEMPLATE =
            Joiner.on("\n\t")
                    .join(
                            ...
                            "     B1.auction,",
                            "     count(*) AS num,",
                            "     HOP_START(B1.eventTime, INTERVAL '%1$d' SECOND, INTERVAL '%2$d' SECOND) AS starttime",
                            "   FROM %3$s B1 ",
                            "   GROUP BY ",
                            "     B1.auction,",
                            "     HOP(B1.eventTime, INTERVAL '%1$d' SECOND, INTERVAL '%2$d' SECOND)",
                            " ) AS AuctionBids",
                            " JOIN (",
                            "   SELECT ",
                            ...
                            "     SELECT",
                            "       count(*) AS num,",
                            "       HOP_START(B2.eventTime, INTERVAL '%1$d' SECOND, INTERVAL '%2$d' SECOND) AS starttime",
                            "     FROM %3$s B2 ",
                            "     GROUP BY ",
                            "       B2.auction, ",
                            "       HOP(B2.eventTime, INTERVAL '%1$d' SECOND, INTERVAL '%2$d' SECOND)",
                            "     ) AS CountBids",
